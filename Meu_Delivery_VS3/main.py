...
def listar_clientes():
    """Retorna todos os clientes para o main.py"""
    conn = conectar()
    cursor = conn.cursor()
...
def listar_itens():
    """Retorna todos os itens para o main.py"""
    conn = conectar()
    cursor = conn.cursor()
...
def listar_pedidos():
    """Retorna lista simplificada de pedidos para a Treeview do Main"""
    conn = conectar()
    cursor = conn.cursor()
    query = """
...
def listar_pedidos_detalhados():
    """Retorna pedidos com informações completas de endereço"""
    conn = conectar()
    cursor = conn.cursor()
    query = """
    SELECT p.id, c.nome, p.valor_total, p.data, p.status_pedido, 
...
if __name__ == "__main__":
    criar_tabelas()
    print("Banco de dados deliveryVS3.db estruturado com sucesso!")